...
namespace PlataformaRio2C.Application.CQRS.CommandsHandlers
{
    ...
    public class UpdateMusicBusinesRoundNegotiationBaseCommandHandler : MusicBusinesRoundNegotiationBaseCommandHandler, IRequestHandler<UpdateMusicBusinessRoundNegotiation, AppValidationResult>
    {
        private readonly IOrganizationRepository organizationRepo;
        private readonly IProjectRepository projectRepo;
        private readonly IMusicBusinessRoundProjectRepository _musicprojectRepo;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="UpdateMusicBusinessRoundNegotiationBaseCommandHandler"/> class.
        /// </summary>
        ...
        public UpdateMusicBusinesRoundNegotiationBaseCommandHandler(
            IMediator eventBus,
            IUnitOfWork uow,
        ...
        public async Task<AppValidationResult> Handle(UpdateMusicBusinessRoundNegotiation cmd, CancellationToken cancellationToken)
        {
            ...
            var project = await this._musicprojectRepo.GetAsync(cmd.ProjectUid ?? Guid.Empty);
            ...
            var automaticScheduledNegotiationsInThisRoom = await this._musicBusinessRoundNegotiationRepository.FindAutomaticScheduledNegotiationsByRoomIdAsync(negotiationRoomConfig?.Room?.Id ?? 0);
            List<MusicBusinessRoundNegotiation> automaticScheduledNegotiations = automaticScheduledNegotiationsInThisRoom.Cast<MusicBusinessRoundNegotiation>().ToList();
            var manualScheduledNegotiationsInThisRoom = await this._musicBusinessRoundNegotiationRepository.FindManualScheduledNegotiationsByRoomIdAsync(negotiationRoomConfig?.Room?.Id ?? 0);
            ...
            {
                automaticScheduledNegotiationsInThisRoom = await this._musicBusinessRoundNegotiationRepository.FindAutomaticScheduledNegotiationsByRoomIdAsync(negotiationRoomConfig?.Room?.Id ?? 0);
            ...
            // Negotiations checks
            var scheduledNegotiationsAtThisTime = await this._musicBusinessRoundNegotiationRepository.FindAllScheduledNegotiationsDtosAsync(cmd.EditionId.Value, null, startDatePreview, endDatePreview);
            ...
            var hasProducerScheduledNegotiationsAtThisTime = scheduledNegotiationsAtThisTime.Count(ndto => (bool)ndto.ProjectBuyerEvaluationDto.MusicBusinessRoundProjectDto.SellerAttendeeOrganizationDto) > 0;
            ...
            this._musicBusinessRoundNegotiationRepository.Update(negotiation);
