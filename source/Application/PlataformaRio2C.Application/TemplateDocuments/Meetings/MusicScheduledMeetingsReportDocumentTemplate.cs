...
namespace PlataformaRio2C.Application.TemplateDocuments
{
    ...
    public class MusicScheduledMeetingsReportDocumentTemplate : TemplateBase
    {
        public readonly List<MusicBusinessRoundNegotiationReportGroupedByDateDto> _negotiationReportGroupedByDateDtos;
        ...
        public MusicScheduledMeetingsReportDocumentTemplate(List<MusicBusinessRoundNegotiationReportGroupedByDateDto> negotiationReportGroupedByDateDtos, string languageCode, EditionDto editionDto)
        {
            ...
            if (this._negotiationReportGroupedByDateDtos?.Any() == true)
            {
                foreach (var negotiationReportGroupedByDateDto in this._negotiationReportGroupedByDateDtos)
                {
                    ...
                    foreach (var negotiationReportGroupedByRoomDto in negotiationReportGroupedByDateDto.MusicBusinessRoundNegotiationReportGroupedByRoomDtos)
                    {
                        ...
                        foreach (var negotiationReportGroupedByStartDateDto in negotiationReportGroupedByRoomDto.MusicBusinessRoundNegotiationReportGroupedByStartDateDtos)
                        {
                            ...
                            string roundStartDate = $"{Labels.Round} {negotiationReportGroupedByStartDateDto.MusicBusinessRoundNegotiations.FirstOrDefault().RoundNumber} ({negotiationReportGroupedByStartDateDto.StartDate.ToString("HH:mm")} - {negotiationReportGroupedByStartDateDto.EndDate.ToString("HH:mm")})";
                            ...
                            foreach (var negotiation in negotiationReportGroupedByStartDateDto.MusicBusinessRoundNegotiations)
                            {
                                table.AddCell(this.GetTableDetails(negotiation, negotiation.Uid == negotiationReportGroupedByStartDateDto.MusicBusinessRoundNegotiations.Last().Uid));
                            }
                        }
                    }
                }
        ...
        private PdfPTable GetTableDetails(MusicBusinessRoundNegotiation negotiation, bool isLastItem)
        {
            ...
            this.AddDetailsRow(ref negotiationsTable, Labels.Player, negotiation.MusicBusinessRoundProjectBuyerEvaluation?.BuyerAttendeeOrganization?.Organization?.TradeName, _fontText);
            this.AddDetailsRow(ref negotiationsTable, Labels.Participant, negotiation.MusicBusinessRoundProjectBuyerEvaluation?.MusicBusinessRoundProject?.SellerAttendeeCollaborator.Collaborator.GetStageNameOrBadgeOrFullName(), _fontText);
