...
namespace PlataformaRio2C.Infra.CrossCutting.SalesPlatforms
{
    ...
    public class SalesPlatformServiceFactory : ISalesPlatformServiceFactory
    {
        ...
        /// </summary>
        /// <param name="salePlatformName">Name of the sale platform.</param>
        /// <returns></returns>
        /// <exception cref="PlataformaRio2C.Infra.CrossCutting.Tools.Exceptions.DomainException">Unknown sales platform on webhook request.</exception>
        public ISalesPlatformService Get(AttendeeSalesPlatformDto attendeeSalesPlatformDto)
        {
            if (attendeeSalesPlatformDto?.SalesPlatform?.Name == SalePlatformName.Eventbrite)
            {
                return new EventbriteSalesPlatformService(attendeeSalesPlatformDto);
            }

            if (attendeeSalesPlatformDto?.SalesPlatform?.Name == SalePlatformName.Inti)
            {
                return new IntiSalesPlatformService(attendeeSalesPlatformDto);
            }

            if (attendeeSalesPlatformDto?.SalesPlatform?.Name == SalePlatformName.Sympla)
            {
                return new SymplaSalesPlatformService(attendeeSalesPlatformDto);
            }
