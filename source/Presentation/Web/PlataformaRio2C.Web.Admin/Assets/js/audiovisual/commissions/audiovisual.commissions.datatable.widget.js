...
// ***********************************************************************
// <copyright file="audiovisual.commissions.datatable.widget.js" company="Softo">
//     Copyright (c) Softo. All rights reserved.
// </copyright>
// <summary></summary>
// ***********************************************************************

var AudiovisualCommissionsDataTableWidget = function () {

    var widgetElementId = '#AudiovisualCommissionsDataTableWidget';
    var tableElementId = '#audiovisual-commissions-list-table';
    ...
    var sendInvitationEmails = function () {
        ...
        jsonParameters.selectedCollaboratorsUids = $('#audiovisual-commissions-list-table_wrapper tr.selected').map(function () { return $(this).data('id'); }).get().join(',');

        $.post(MyRio2cCommon.getUrlWithCultureAndEdition('/Audiovisual/Commissions/SendInvitationEmails'), jsonParameters, function (data) {
    ...
    var initiListTable = function () {
        ...
        table = tableElement.DataTable({
            ...
            ajax: {
                url: MyRio2cCommon.getUrlWithCultureAndEdition('/Audiovisual/Commissions/Search'),
                data: function (d) {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        if (!full.IsInCurrentEdition) {
                            html += '<button class="dropdown-item" onclick="AudiovisualCommissionsUpdate.showModal(\'' + full.Uid + '\', true);"><i class="la la-plus"></i> ' + addToEdition + '</button>';
                        }
                        else {
                            html += '<button class="dropdown-item" onclick="AudiovisualCommissionsDataTableWidget.showDetails(\'' + full.Uid + '\', false);"><i class="la la-eye"></i> ' + labels.view + '</button>';
                        }

                        if (full.IsInCurrentEdition && full.IsInOtherEdition) {
                            html += '<button class="dropdown-item" onclick="AudiovisualCommissionsDelete.showModal(\'' + full.Uid + '\', true);"><i class="la la-minus"></i> ' + removeFromEdition + '</button>';
                        }
                        else {
                            html += '<button class="dropdown-item" onclick="AudiovisualCommissionsDelete.showModal(\'' + full.Uid + '\', false);"><i class="la la-remove"></i> ' + labels.remove + '</button>';
                        }
    ...
    var showDetails = function (commissionUid) {
        ...
        window.location.href = MyRio2cCommon.getUrlWithCultureAndEdition('/Audiovisual/Commissions/Details/' + commissionUid);
    };
