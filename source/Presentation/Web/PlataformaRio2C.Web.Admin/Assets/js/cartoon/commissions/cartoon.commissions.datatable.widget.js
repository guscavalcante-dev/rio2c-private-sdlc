...
var CartoonCommissionsDataTableWidget = function () {

    var widgetElementId = '#CartoonCommissionsDataTableWidget';
    var tableElementId = '#cartoon-commissions-list-table';
    ...
    var sendInvitationEmails = function () {
        ...
        jsonParameters.selectedCollaboratorsUids = $('#cartoon-commissions-list-table_wrapper tr.selected').map(function () { return $(this).data('id'); }).get().join(',');

        $.post(MyRio2cCommon.getUrlWithCultureAndEdition('/Cartoon/Commissions/SendInvitationEmails'), jsonParameters, function (data) {
        ...
        table = tableElement.DataTable({
            ...
            buttons: [
            {
                ...
                buttons: [
                    ...
                    //        eventbriteCsvExport.selectedCollaboratorsUids = $('#cartoon-commissions-list-table_wrapper tr.selected').map(function () { return $(this).data('id'); }).get().join(',');
            ...
            ajax: {
                url: MyRio2cCommon.getUrlWithCultureAndEdition('/Cartoon/Commissions/Search'),
                data: function (d) {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        if (!full.IsInCurrentEdition) {
                            html += '<button class="dropdown-item" onclick="CartoonCommissionsUpdate.showModal(\'' + full.Uid + '\', true);"><i class="la la-plus"></i> ' + addToEdition + '</button>';
                        }
                        else {
                            html += '<button class="dropdown-item" onclick="CartoonCommissionsDataTableWidget.showDetails(\'' + full.Uid + '\', false);"><i class="la la-eye"></i> ' + labels.view + '</button>';
                        }

                        if (full.IsInCurrentEdition && full.IsInOtherEdition) {
                            html += '<button class="dropdown-item" onclick="CartoonCommissionsDelete.showModal(\'' + full.Uid + '\', true);"><i class="la la-minus"></i> ' + removeFromEdition + '</button>';
                        }
                        else {
                            html += '<button class="dropdown-item" onclick="CartoonCommissionsDelete.showModal(\'' + full.Uid + '\', false);"><i class="la la-remove"></i> ' + labels.remove + '</button>';
                        }
    ...
    var showDetails = function (commissionUid) {
        ...
        window.location.href = MyRio2cCommon.getUrlWithCultureAndEdition('/Cartoon/Commissions/Details/' + commissionUid);
    };
