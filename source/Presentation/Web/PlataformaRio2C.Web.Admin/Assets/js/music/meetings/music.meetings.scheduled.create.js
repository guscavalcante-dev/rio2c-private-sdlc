...
var MusicMeetingsCreate = function () {
    ...
    var toogleProjectSelect2 = function () {
        ...
        if (!MyRio2cCommon.isNullOrEmpty(buyerOrganizationUidElement.val())) {
            enableProjectSelect2(true);
            ...
            $(projectUid).val('').trigger('change');
        }
    };

    var enableProjectSelect2 = function (isParentChanged) {
        if (!MyRio2cCommon.isNullOrEmpty(isParentChanged) && isParentChanged === true) {
            if (!MyRio2cCommon.isNullOrEmpty(projectUidElement.val())) {
                projectUidElement.val('').trigger('change');
            }
            else {
                projectUidElement.val('');
            }
        }
    ...
    var enableProjectChangeEvent = function () {
        ...
        //    toogleDateSelect2();
        //    MusicMeetingsLogisticsInfoWidget.init([this.value, buyerOrganizationUidElement.val()]);
        //});
    ...
    var toogleDateSelect2 = function () {
        var element = $(negotiationConfigUid);

        if (!MyRio2cCommon.isNullOrEmpty(projectUidElement.val())) {
            enableDateSelect2(true);
            element.removeClass('disabled');
            element.prop("disabled", false);
        }
        else {
            element.addClass('disabled');
            element.prop("disabled", true);
            $(negotiationConfigUid).val('').trigger('change');
        }
    };
    ...
    var showModal = function () {
        ...
        $.get(MyRio2cCommon.getUrlWithCultureAndEdition('/Music/Meetings/ShowCreateModal'), jsonParameters, function (data) {
            MyRio2cCommon.handleAjaxReturn({
                data: data,
                // Success
                onSuccess: function () {
