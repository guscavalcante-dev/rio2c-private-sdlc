...
var MusicCommissionsDataTableWidget = function () {
    ...
    var sendInvitationEmails = function () {
        ...
        jsonParameters.selectedCollaboratorsUids = $('#music-commissions-list-table_wrapper tr.selected').map(function () { return $(this).data('id'); }).get().join(',');

        $.post(MyRio2cCommon.getUrlWithCultureAndEdition('/Music/Commissions/SendInvitationEmails'), jsonParameters, function (data) {
    ...
    var showExportEventbriteCsvModal = function () {
        ...
        $.get(MyRio2cCommon.getUrlWithCultureAndEdition('/Music/Commissions/ShowExportEventbriteCsvModal'), jsonParameters, function (data) {
            MyRio2cCommon.handleAjaxReturn({
                data: data,
                // Success
                onSuccess: function () {
    ...
    var exportEventbriteCsv = function () {
        ...
        $.get(MyRio2cCommon.getUrlWithCultureAndEdition('/Music/Commissions/ExportEventbriteCsv'), eventbriteCsvExport, function (data) {
            MyRio2cCommon.handleAjaxReturn({
                data: data,
                // Success
                onSuccess: function () {
    ...
    var initiListTable = function () {
        ...
        table = tableElement.DataTable({
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        action: function (e, dt, node, config) {
                            ...
                            eventbriteCsvExport.selectedCollaboratorsUids = $('#music-commissions-list-table_wrapper tr.selected').map(function () { return $(this).data('id'); }).get().join(',');
            ...
            ajax: {
                url: MyRio2cCommon.getUrlWithCultureAndEdition('/Music/Commissions/Search'),
                data: function (d) {
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        if (full.IsInCurrentEdition && full.IsInOtherEdition) {
                            html += '<button class="dropdown-item" onclick="MusicCommissionsDelete.showModal(\'' + full.Uid + '\', true);"><i class="la la-plus"></i> ' + removeFromEdition + '</button>';
                        }
                        else {
                            html += '<button class="dropdown-item" onclick="MusicCommissionsDelete.showModal(\'' + full.Uid + '\', false);"><i class="la la-remove"></i> ' + labels.remove + '</button>';
                        }
